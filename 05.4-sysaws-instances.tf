resource "aws_instance" "app3" {
   ...
   depends_on = ["aws_ebs_volume.app3"]
  ...
  connection {
    ...
    agent = false
    host = "${aws_instance.app3.public_ip}"
  }
...
resource "aws_volume_attachment" "app3" {
  device_name = "/dev/sdb"
  volume_id   = "${aws_ebs_volume.app3.id}"
  instance_id = "${aws_instance.app3.id}"
}
...
resource "aws_route53_record" "app3" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.app3.public_ip}"]
}
